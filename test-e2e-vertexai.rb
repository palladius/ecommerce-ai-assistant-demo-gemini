...
llm = Langchain::LLM::GoogleVertexAI.new(
  project_id: ENV['GOOGLE_PROJECT_ID'], # eg, 'palladius-genai'. You get it with your GCP onboarding.
  region: 'europe-west1', # HardCoded bcs done last minute - sorry!
#  region: 'us-east1',
  default_options: { chat_completion_model_name: CHAT_COMPLETION_MODEL_NAME }
)

puts("♊️ Gemini model: #{CHAT_COMPLETION_MODEL_NAME}")
puts("♊️ Google ProjectId: #{GOOGLE_PROJECT_ID}")
