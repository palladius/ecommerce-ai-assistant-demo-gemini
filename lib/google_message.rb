  ...
  module Messages
    class GoogleGeminiMessage
      def to_s
        ...
        # model
        return "🤖 [modl] 💬 #{sanitized_content.colorize :cyan}" if role == 'model'
        # user
        return "🧑 [user] 💬 #{sanitized_content.colorize :yellow}" if role == 'user'
        # function 🖩 would be great but too small 🧮 this is nice
        if role == 'function'
          return "🔢 [func] 🛠️  #{tool_call_id.force_encoding('UTF-8').colorize :white} => #{sanitized_content.colorize :gray}"
        end
